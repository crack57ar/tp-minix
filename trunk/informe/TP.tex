...
void filosofoNoUltimo(int i)
{
	...
	while (contador > 0)
	{
		...
		/* de la derecha */
		printf("- %i quiere tomar el palito de la izquierda \n",i);
		semP(sem_names[i]);
		printf("- %i quiere tomar el palito de la derecha \n",i);
		semP(sem_names[i+1]);
		
		come(i);
		/* Liberamos los palitos en el orden inverso */
		semV(sem_names[i + 1]);
		printf("- %i dejo el palito de la derecha \n",i);
		semV(sem_names[i]);
		printf("- %i dejo el palito de la izquierda \n",i);
...
\newpage	
	...
	\begin{itemize}
		\item \verb@/tests/lectores/resultado.txt@ - Resultados del test de Lectores-Escritores
		\item \verb@/usr/srcTpFinal/lectores.c@: - Implementación de la solución al problema con test e información del proceso paso a paso.
	\end{itemize}

	\begin{itemize}
		\item \verb@/tests/filosofos/filosofos.out@ - Resultados del test del problema de los filósofos chinos
		\item \verb@/usr/srcTpFinal/filosofos.c@: Implementación de la solución al problema con test e información del proceso paso a paso.
	\end{itemize}	
...
\begin{itemize}
	...
	\item \verb@/tests/inode/tty0.txt@: Salida de prodcons1. Salida parecida al anterior caso, sólo que para otro tipo de dispositivo, en este caso, un dispositivo de caracteres.
	\item \verb@/tests/inode/inode.txt@: Salida del archivo mismo que estoy ejecutando.
	\item \verb@/usr/srcTpFinal/inode.c@: Implementación del programa que muestra un inodo por pantalla.
\end{itemize}
\newpage
